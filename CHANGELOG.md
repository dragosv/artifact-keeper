...
- **`docker/init-dtrack.sh` re-applies DT team permission grants on every restart, not only on fresh installs** (closes #1530). The grant loop added in #1511 lived after `dtrack-init`'s fast-path early-exit, which short-circuits with `exit 0` when `/shared/dtrack-api-key` already exists from a previous run. Deployments that minted their API key on a pre-#1511 build therefore had a valid key but a team with none of the four required permissions, so every PUT `/api/v1/project` from the scan pipeline returned 403 even after the operator upgraded the backend image. The fix replaces the unconditional `exit 0` with a `KEY_ALREADY_PROVISIONED=true` flag that skips only the rotate/mint block; auth, team lookup, and the four POST `/api/v1/permission/{permission}/team/{uuid}` grants now run on every boot. The DT permission endpoint is idempotent (200 on first grant, 304 thereafter) so re-running on the warm path is cheap and silent. A new assertion in `docker/test-init-dtrack.sh` Phase 2 (warm restart) pins the regression: the test mock now tracks the set of permissions granted per run, and the warm-restart phase must record eight POSTs total (four from cold start + four from warm restart). The same test fails on `main` with `POST count=4, expected 8`. Operators on the bundled compose / Helm path with a stuck DT integration self-repair by restarting the `dtrack-init` container; operators who bring their own DT instance still need the manual grant call documented in #1511.
